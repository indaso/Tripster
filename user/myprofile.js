    ...
    oracle.connect(connectData, function (err, connection) {
      ...
      connection.execute(query, [], function (err, results) {
        ...
        if (results.length === 0) {
          ...
          var mess = "Sorry, we were not able to find: " + friendee +
            " in out database. Please try again";
          res.render("addfriends", {
            errormsg: mess
          });
          ...
          oracle.connect(connectData, function (err, connection) {
            ...
            friended = friended + "OR (FRIEND_ID1 = '" + friendee + "' AND FRIEND_ID2 = '" +
              friender + "'";
            ...
            if (results.length === 0) {
              ...
              oracle.connect(connectData, function (err, connection) {
                ...
                //Place friend request in database
                var createreq =
                  "INSERT INTO FRIENDS_WITH (FRIEND_ID1, FRIEND_ID2, ACCEPTED) VALUES";
...
router.post('/friendrequests', function (req, res) {
  ...
  if (response == "accept") {
    //Validate that person you are trying to friend exists
    var query = "UPDATE FRIENDS_WITH SET ACCEPTED=1 WHERE FRIEND_ID1='" + friender + "' AND FRIEND_ID2='" +
      friendee + "'";
    oracle.connect(connectData, function (err, connection) {
      if (err) {
        console.log("Error connecting to db:", err);
